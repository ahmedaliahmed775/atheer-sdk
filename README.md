...
*   **معالجة الدفع دون اتصال بالإنترنت**: تدعم المعاملات حتى عندما يكون الجهاز غير متصل بالإنترنت، باستخدام الرموز المميزة المجهزة مسبقًا وخزنة محلية آمنة، مع مزامنة تلقائية بمجرد استعادة الاتصال بالإنترنت.
*   **دعم القسائم الرقمية (Vouchers)**: تدعم المكتبة الآن القسائم الرقمية بمختلف أطوالها (مثل أكواد جوالي المكونة من 7 أرقام)، مما يضمن التوافق التام مع أنظمة 'جوالي' و 'فلوسك'.
*   **نفق اتصال مجاني عبر APN (Zero-rated Data Tunneling)**: صُممت معمارية الـ SDK لتوجيه المعاملات عبر APN مخصص لضمان عملها بدون الحاجة لرصيد إنترنت قياسي. تم تغليف هذه الآلية في وحدة مستقلة (`AtheerCellularRouter.kt`) وهي جاهزة للتفعيل بمجرد إتمام الشراكات مع مزودي خدمة الاتصالات.
*   **إجراءات أمان متقدمة**:
...
*   **وحدة الأمان (`com.atheer.sdk.security`)**:
    *   **AtheerKeystoreManager**: يدير مفاتيح التشفير باستخدام Android Keystore، ويقوم بالتشفير وفك التشفير وترميز البيانات الحساسة (مثل أرقام البطاقات). كما يتعامل مع توليد Nonce لمنع هجمات إعادة التشغيل. يدعم التشفير الآن أطوالاً متغيرة للقسائم الرقمية (Vouchers).
    *   **AtheerTokenManager**: ينفذ خزنة رموز مميزة دون اتصال بالإنترنت باستخدام `EncryptedSharedPreferences` لتخزين رموز الدفع المجهزة مسبقًا بأمان. يدعم الآن القسائم الرقمية بمختلف أطوالها لضمان التوافق مع المحافظ اليمنية.
*   **وحدة الشبكة (`com.atheer.sdk.network`)**:
    *   **AtheerNetworkRouter**: يتعامل مع جميع الاتصالات الشبكية الآمنة مع الواجهة الخلفية لـ Atheer. يستخدم `OkHttp` مع ميزات متقدمة مثل **تثبيت الشهادة (Certificate Pinning)** لمنع هجمات Man-in-the-Middle (MITM) ويفرض TLS 1.2+ للاتصالات الآمنة. يتضمن الآن نظاماً موحداً لترجمة ردود السيرفر إلى `AtheerError`.
    *   **AtheerCellularRouter**: وحدة مخصصة (معلقة حالياً) لإدارة الاتصال عبر نفق APN مخصص لضمان الوصول المجاني (Zero-rated) لخدمات الدفع.
...
| الطريقة                                   | الوصف                                                                                                                                      |
| :--------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
| `init(context, merchantId, apiBaseUrl, enableApnFallback)`  | **(ثابت)** تهيئة SDK بمعرف التاجر ورابط مقسم أثير (Atheer Switch Gateway). يقوم بإجراء فحوصات أمان ويضبط خيار التوجيه عبر APN. |
...
| `charge(request, accessToken)`           | **(تعليق)** ينفذ عملية شحن مباشرة عبر الإنترنت ضد الواجهة الخلفية لـ Atheer. يدعم حقول المحافظ اليمنية (انظر كائن الطلب). |
...
### كائن الطلب `ChargeRequest` (المحافظ اليمنية)

عند استخدام دالة `charge` للمحافظ اليمنية، يدعم كائن `ChargeRequest` الحقول التالية:

| الحقل | النوع | الوصف |
| :--- | :--- | :--- |
| `agentWallet` | `String?` | معرف محفظة الوكيل المسؤول عن العملية. |
| `receiverMobile` | `String?` | رقم هاتف المستلم النهائي للأموال. |
| `externalRefId` | `String?` | رقم المرجع الخاص بنظام المحفظة المضيفة (التتبع الخارجي). |
| `purpose` | `String?` | الغرض من العملية (مثال: دفع خدمات، تحويل). |
| `providerName` | `String?` | اسم مزود الخدمة (مثال: JAWALI, KURAIMI). |

## معالجة الردود والأخطاء

### حالات الاستجابة (`status`)
تدعم المكتبة حالات الاستجابة القياسية التالية والمتوافقة مع أنظمة المحافظ الإلكترونية:
- **`ACCEPTED`**: تم قبول العملية ومعالجتها بنجاح.
- **`PENDING`**: العملية بانتظار إجراء إضافي (مثل إدخال رمز التحقق OTP أو تأكيد من جهة خارجية).
- **`REJECTED`**: تم رفض العملية من قبل نظام المحفظة أو البنك (بسبب الرصيد، الحظر، إلخ).
- **`REVERSED`**: تم عكس العملية (استرداد المبلغ) بنجاح.
- **`EXPIRED`**: انتهت صلاحية الطلب قبل إتمام عملية الدفع.

### نظام الأخطاء الموحد `AtheerError`
يمكن للمطورين استخدام الفئة المختومة `AtheerError` لتوجيه المستخدم بشكل دقيق بناءً على سبب الفشل:

```kotlin
when (error) {
    is AtheerError.InsufficientFunds -> showToast("رصيد محفظتك غير كافٍ، يرجى الشحن والمحاولة مرة أخرى.")
    is AtheerError.InvalidVoucher -> showToast("رمز القسيمة غير صحيح أو انتهت صلاحيته.")
    is AtheerError.ProviderTimeout -> showToast("تأخر الرد من مزود الخدمة (مثل جوالي/الكريمي)، يرجى التحقق من حالة العملية لاحقاً.")
    is AtheerError.NetworkError -> showToast("يرجى التحقق من اتصال الإنترنت أو إعدادات APN أثير.")
    is AtheerError.AuthenticationFailed -> Log.e("AtheerSDK", "خطأ في مصادقة التاجر/الوكيل.")
    else -> showToast("حدث خطأ غير معروف، يرجى المحاولة مرة أخرى.")
}
```
...
#### منع التهديدات
*   **الحماية من هجمات إعادة التشغيل (Replay Attack Protection)**: تصاحب كل معاملة **Nonce** فريد يستخدم لمرة واحدة يتم إنشؤه بواسطة `AtheerKeystoreManager`. يتحقق خادم الواجهة الخلفية من هذا Nonce لضمان عدم إمكانية إعادة تشغيل طلب معاملة تم التقاطه بشكل ضار.
...
*   **`AtheerSdk.init(context, merchantId, apiBaseUrl, enableApnFallback)`**: يتم التكوين الأساسي أثناء التهيئة:
    *   `merchantId`: معرفك الفريد كتاجر، مقدم من Atheer.
    *   `apiBaseUrl`: يمثل "رابط مقسم أثير المالي (Atheer Switch Gateway)" والذي يقوم باستقبال المعاملات وتوجيهها لخوادم البنوك.
    *   `enableApnFallback`: قيمة منطقية (true/false) توجه الـ SDK لفتح اتصال خلوي مباشر (Cellular Network Request) بمقسم أثير عند غياب الواي فاي والإنترنت القياسي.
